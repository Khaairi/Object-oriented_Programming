...
public class Game extends Canvas implements Runnable
{
    ...
    public Game()
    {
        try
        {
            ...
            if(running)
            {
                ...
                handler.add(new Obstacle(rand.nextInt(480), rand.nextInt(640)));
    ...
    public void render()
    {
        ...
        if(running == true)
        {
            ...
            g.setColor(Color.blue);
            g.drawString("Catch : " + Integer.toString(eat), 20, 50);
...
//        System.out.println(handler.get(0).getX());
        handler.loop();
        if(this.running)
        {   
            ...
            for(int i = 0; i < handler.count(); i++)
            {
                ...
                {
                    if(handler.get(0).getX() == handler.get(i).getX() && handler.get(0).getY() == handler.get(i).getY())
                    {
                        handler.remove(i);
                        handler.add(new Obstacle(rand.nextInt(480), rand.nextInt(640)));
                        this.eat = this.eat + 1;
